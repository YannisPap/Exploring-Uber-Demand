Exploring UBER demand in NYC by Yannis Pappas
========================================================
...
uber <- read_csv("uber.csv", col_types = cols(
  borough = col_factor(levels = c("Bronx", "Brooklyn", "EWR", "Manhattan", "Queens", "Staten Island", "Unknown")), 
...
```{r}
uber.spread <- uber %>% spread(borough, pickups)
```


```{r}
d <- melt(uber.spread[2:10])
...
```{r}
h1 <- ggplot(uber.spread, aes(pcp01)) +
  geom_histogram() +
  scale_x_sqrt() +
  scale_y_sqrt() +
  coord_cartesian(xlim = c(0, 2.1), ylim = c(0, 4000))

h2 <- ggplot(uber.spread, aes(pcp06)) +
  geom_histogram() +
  scale_x_sqrt() +
  scale_y_sqrt() +
  coord_cartesian(xlim = c(0, 2.1), ylim = c(0, 4000))

h3 <- ggplot(uber.spread, aes(pcp24)) +
  geom_histogram() +
  scale_x_sqrt() +
  scale_y_sqrt() +
  coord_cartesian(xlim = c(0, 2.1), ylim = c(0, 4000))
...
We can see how the histogram is transformed by the built up of the values because of the summation. 
...
uber.spread <- uber.spread %>% 
  mutate(pickups = Bronx +Brooklyn + EWR + Manhattan + Queens + `Staten Island` + !is.na(`<NA>`)) %>% 
  mutate(day = day(pickup_dt)) %>%
  mutate(hour = hour(pickup_dt)) %>%
  mutate(wday = wday(pickup_dt, label = TRUE)) %>% 
  mutate(workday = ifelse(wday == 'Sat' | wday == 'Sun' | hday == 'Y', 'N', 'Y')) %>% 
...
pairs.panels(uber.spread[ , c(1, 11:23)])
```
...
```{r, fig.width=12, fig.height=8}
pairs.panels(uber.spread[ , c(2:10, 12:19)])
```
...
ggplot(uber.spread, aes(yday, pickups)) +
  ...
  geom_line(stat = 'summary', fun.y = quantile, fun.args = list(probs = 0.75), linetype = 2, color = 'blue') +
  geom_smooth() +
  scale_x_continuous(breaks = c('1 Jan.' = 0, '1 Feb.' = 31, '1 Mar.' = 59, '1 Apr.' = 90, '1 May' = 120, '1 Jun.' = 151, '30 Jun.' = 181))
...
```
```{r}
mean((uber %>% filter(borough == 'Brooklyn') %>% filter(workday == 'N'))$pickups) / mean((uber %>% filter(workday == 'N'))$pickups)
```
...
```{r}
m1 <- lm(pickups ~ pickup_dt + hour + wday + workday + borough, data = uber.major[169:16700,])
m2 <- update(m1, ~ . + `1w`)
m3 <- update(m2, ~ . + `1d`)
m4 <- update(m3, ~ . + `3h`)
m5 <- update(m4, ~ . + `2h`)
...
```{r}
m1 <- lm(pickups ~ pickup_dt + hour + wday + workday, data = manhattan[169:4175,])
...
test <- test %>% gather('model', 'prediction', 15:20)

ggplot(test, aes(pickup_dt, pickups)) +
  geom_point() +
  geom_line(aes(pickup_dt, prediction, color = model)) +
  scale_x_datetime(date_breaks = '1 day', date_labels = '%a')
```

```{r, fig.width=12, fig.height=8}
test$wday <- factor(test$wday, levels = c('Wed','Thurs','Fri','Sat','Sun','Mon','Tues'))
...
ggplot(manhattan, aes(hour, pickups)) +
  ...
  geom_line(data = subset(manhattan, yday == 178), aes(hour,pickups,color = 'Saturday June 27'), )
```
...
### Plot One
```{r, fig.width=12, fig.height=8}
uber$borough <- factor(uber$borough, levels = c('Manhattan', 'Brooklyn', 'Queens', 'Bronx', 'Staten Island', 'EWR'))

ggplot(subset(uber, !is.na(borough)), aes(pickups)) +
  geom_histogram(aes(fill = borough), bins = 40) +
  scale_x_sqrt() +
  facet_wrap(~ borough, ncol = 2, scales = 'free') +
  labs(title = 'Pickups per hour distribution by borough', x = 'Pickups per hour', y = 'Count') +
  theme(plot.title = element_text(size = 22, hjust = 0.5), legend.position = 'none', axis.title = element_text(size = 16))
```
...
ggplot(test, aes(pickup_dt, pickups)) +
  ...
  labs(title = "Models' Performance", x = 'Time', y = 'Pickups', color = 'Model') +
  theme(plot.title = element_text(size = 22, hjust = 0.5), axis.title = element_text(size = 16))
```
