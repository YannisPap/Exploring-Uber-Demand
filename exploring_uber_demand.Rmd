...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE,
                      ...
                      fig.path = '../Figures/Exploring-Uber-Demand/')
```
...
manhattan <- uber %>%  
  ...
  dplyr::select(pickup_dt:pickups, hour, wday, workday) %>%  
  mutate(`1h` = lag(pickups)) %>% 
  mutate(`2h` = lag(pickups,2)) %>% 
  mutate(`3h` = lag(pickups,3)) %>% 
  mutate(`1d` = lag(pickups,24)) %>% 
...
In general we have well balanced residuals (except m1) which is a strong indication that our models are optimised.  
